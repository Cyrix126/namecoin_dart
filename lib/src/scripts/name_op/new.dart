...
Future<(String, String, String)> scriptNameNew(String name,
    [Uint8List? privkey]) async {
  final identifier = ascii.encode(name);
  validateIdentifierLength(identifier);
  final (nameOp, salt) = await _buildNameNew(identifier, privkey);
...
Future<(Map<String, Object>, Uint8List)> _buildNameNew(Uint8List identifier,
    [Uint8List? privkey]) async {
  final Uint8List salt;
  if (privkey != null) {
    salt = await saltFromPrivKey(privkey, identifier);
  } else {
...
Future<Uint8List> saltFromPrivKey(
  Uint8List privkey,
  ...
  final secretKey = SecretKey(privkey);
  final info = ascii.encode('Namecoin Registration Salt');
  final salt =
      await hkdf.deriveKey(secretKey: secretKey, nonce: identifier, info: info);
  return Uint8List.fromList(salt.bytes.sublist(0, 20));
}
